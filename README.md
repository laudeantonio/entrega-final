...
Nodo: Extracción_información_no_sensible (Set):

Qué hace: Filtra y organiza los datos necesarios para la IA (cargo, empresa, ciudad, fuente, industria y número de empleados), omitiendo intencionalmente datos personales como nombres o teléfonos por seguridad.

Nodo: Base_Datos_Sheets (Google Sheets):
...
Nodos: Consulta_BD_Asesores_Activos (Supabase):

Qué hace: Busca en la tabla asesores a aquellos que estén marcados como activos y cuya especialidad coincida con el tier del lead.

Nodo: Selección_Asesor (Code):
...
Nodo: Traductor_Ingles / Gmail_Confirmación:

Qué hace: Genera un mensaje comercial personalizado y lo envía al lead vía Gmail. El correo incluye un botón para agendar una demo directamente en Calendly.

Nodo: Actualización_Estado_Sheets (Google Sheets):

Qué hace: Actualiza la fila del lead con el nombre del asesor asignado, la razón del scoring y cambia el estado a "Contactado".
